...
// REPONSE AU CLIENT
    ...
    numfil = htons(numfil);
    nb = htons(0);
    addr_MD=htonl(grp.sin6_addr.s6_addr32[3]);
...
int subscribe_request(int sock_client, char *buf,int port) {
    // TRADUCTION DU MESSAGE DU CLIENT
    uint16_t header, id, numfil, nb,addr_MD;
    uint8_t codereq, lendata;
    char data[SIZE_MESS+1];
...
// REPONSE AU CLIENT
    ...
    memcpy(buf+sizeof(uint16_t)*2, &nb, sizeof(uint16_t));
    memcpy(buf+sizeof(uint16_t)*3, &addr_MD, sizeof(uint16_t));
    recv_send_message(sock_client, buf, sizeof(uint16_t)*4, SEND);
